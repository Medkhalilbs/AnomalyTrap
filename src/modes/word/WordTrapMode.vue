...
function getBadgeText(type: string): string {
  ...
  switch (type) {
    case 'anagram': return `🔤 ${m.anagram || 'Anagram'}`;
    case 'oddOneOut': return `🎯 ${m.oddOneOut || 'Odd One Out'}`;
    case 'category': return `📂 ${m.category || 'Category'}`;
    default: return `🧩 ${m.wordPuzzle || 'Word Puzzle'}`;
  }
}
...
function nextChallenge() {
  ...
  currentChallenge.value = generator.generateChallenge(Math.floor(store.score / 50) + 1, store.language as 'en' | 'fr' | 'ar');
